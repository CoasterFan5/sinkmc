...
Routes are separated as if to use file-based routing, but they are not. 
...
## OpenAPI Schemas
Declare an openapi schema inside of a `schema.ts` file. This makes it simple to identify the schema of a route while also separating the schema from the code behind it. All OpenAPI schemas for routes should be exported with _schema suffix. For instance, getSession -> getSession_schema.
